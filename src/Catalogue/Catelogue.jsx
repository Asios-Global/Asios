...
const Catelogue = () => {
    ...
    const fetchCataloguesBySubcategory = async (cataloguesubcategory) => {
        setCatalogueSubCategories(cataloguesubcategory);
        try {
            const response = await axios.get(`${baseURL}/catalogue/catalogueSubCategory/${cataloguesubcategory}`);
            setCatalogues(response.data.data);
        } catch (error) {
            console.error("Error fetching catalogues by subcategory:", error);
        }
    };
            ...
            <div className='flex md:flex-row flex-col md:gap-10 sm:gap-7 gap-5 py-14'>
                ...
                <div className='flex-1 flex flex-col container md:p-0'>
                    ...
                    <div className="md:p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {catalogues.map((item) => (
                            <div key={item.id} className="relative group mb-4 overflow-hidden rounded-lg shadow-lg transition-all hover:shadow-2xl">
                                <img
                                    src={item.image} 
